...
export type WeaponId = 'LANTERN CORE' | 'LIGHT BLADE' | 'SPEAR OF DAWN' | 'LUMEN BOW' | 'RELIC STAFF';
export type SkinId = 'DAWN' | 'MOSS' | 'VOID';
...
export interface Player extends Vec2 { w:number; h:number; vx:number; vy:number; facing:number; grounded:boolean; coyote:number; jumpBuffer:number; dashTimer:number; dashCooldown:number; attackTimer:number; attackCooldown:number; burstTimer:number; burstCooldown:number; veilTimer:number; veilCooldown:number; health:number; maxHealth:number; lantern:number; maxLantern:number; invuln:number; hurtFlash:number; weapon:WeaponId; skin:SkinId; landTimer?:number; runTime?:number; hurtTimer?:number; }
export interface GameSnapshot {
  ...
  act?:'FEAR'|'DOUBT'|'IDENTITY'; actQuestion?:string; roomTitle?:string; mechanic?:string; lanternMode?:string; memoryMode?:boolean; bossName?:string; bossPhase?:number; endingChoice?:EndingChoice; weapon?:WeaponId; skin?:SkinId;
}
